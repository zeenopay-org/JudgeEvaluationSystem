...
const createJudge = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/v1/judges/create', {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">Add a new judge to the evaluation system</p>
        </div>
      
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Username Field */}
          <div>
            <label htmlFor="judge-username" className="block text-sm font-medium text-gray-700 mb-2">
              Username
            </label>
            <input
              type="text"
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="">
                  {isLoadingEvents ? 'Loading events...' : 'Select an event to assign judge'}
                </option>
                {events.map((event) => (
