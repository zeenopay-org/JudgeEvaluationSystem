    ...
    try {
      const response = await fetch('http://localhost:5000/api/v1/rounds/create', {
        ...
        body: JSON.stringify({
          ...
          max_score,
          eventId,
          questions: type === 'qna' ? questions.filter(q => q.trim() !== '') : []
        })
      });
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Create New Round</h1>
          <p className="text-gray-600 mt-2">Add a new round to the evaluation system</p>
        </div>
      
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Username Field */}
          <div>
                <label htmlFor="round-name" className="block text-sm font-medium text-gray-700 mb-2">
              Round Name
            </label>
            <input
              type="text"
          ...
          <div className="relative">
            ...
            <div className="relative">
              <input
                ...
                className="w-full max-w-full px-3 py-2 pr-12 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                required
              />
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="">
                  {isLoadingEvents ? 'Loading events...' : 'Select an event to assign round'}
                </option>
                {events.map((event) => (
            ...
            {events.length === 0 && !isLoadingEvents && (
              <p className="mt-2 text-sm text-amber-600">No events available. Please create an event first.</p>
            )}
          </div>
